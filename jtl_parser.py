    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        for h in headers:
            ...
            if hl in ['timestamp', 'timeStamp', 'timestamp']:
                ...
                col_map['label'] = h
            elif hl in ['responsecode', 'code', 'status']:
            ...
            elif hl in ['threadname', 'thread']:
                col_map['thread_name'] = h
            elif hl in ['datatype', 'type']:
                col_map['data_type'] = h
            elif hl in ['bytes', 'bodybytes']:
                col_map['bytes'] = h
            ...
            elif hl in ['samplecount', 'sample']:
                col_map['sample_count'] = h
            elif hl in ['errorcount', 'errors']:
                col_map['error_count'] = h
            elif hl in ['allthreads', 'threads']:
                col_map['all_threads'] = h

        for row in reader:
            try:
                ts = int(row.get(col_map.get('timestamp', 'timeStamp'), 0))
                elapsed = int(row.get(col_map.get('elapsed', 'elapsed'), 0))
                ...
                sample = Sample(
                    ...
                    success=row.get(col_map.get('success', 'success'), 'true').lower() == 'true',
    ...
    for elem in root.iter():
        if elem.tag == 'httpSample' or elem.tag == 'sample':
            try:
                ...
                # 获取 URL（优先 u 属性，其次 java.net.URL 子元素）
                ...
                # 获取 queryString 子元素文本（POST body）
                query_string = ''
                ...
                # 如果 queryString 为空，尝试从 URL 中提取（GET 请求参数在 URL 中）
